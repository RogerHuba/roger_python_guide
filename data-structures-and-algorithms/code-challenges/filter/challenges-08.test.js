...
const filterStringsWithVowels = arr => arr.filter( val => (/[aeiou]/i).test(val));
...
const evenOddNumericValues = (arr) => arr.filter(val => typeof val === 'number').map(val => val %2 ? 'odd' : 'even');
