...
const divisibleByFiveTwoToThePower = input => {
  const numbers = input.map(arr => arr.filter(val => typeof(val) === 'number' && !(val %5)));
  return numbers.map(arr => arr.map (val => Math.pow(2, val)));
};
...
let findMaleAndFemale = data => {
  const humans = data.filter( person => person.gender === 'male' || person.gender === 'female');
  return humans.map( person => person.name).join(' and ');
};
...
let findShortest = (data) => {
  ...
  return data.reduce( (name, person) => {
    if(parseInt(person.height) < height) {
