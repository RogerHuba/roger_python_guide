...
const onlyOddChars = (str) => str.split('').filter( (letter, idx) => idx % 2 ).join('');
...
const sortByDay = (arr) => daysOfWeek.map( day => arr.filter( str => str.includes(day) ) );
...
const characterByIndex = (arr) => arr.map( (str, idx) => str.charAt(idx) );
