...
const toTitleCase = (arr) => arr.map( str => str[0].toUpperCase() + str.slice(1));
...
const sortBy = (property, arr) => arr.sort( (a, b) => a[property] === b[property] ? 0 : a[property] < b[property] ? -1 : 1);
...
const detectTicTacToeWin = (board) => {
  const helper = ([[r1, c1], [r2, c2], [r3, c3]]) => {
    return board[r1][c1] === board[r2][c2] && board[r1][c1] === board[r3][c3] && (board[r1][c1] === 'X' || board[r1][c1] === 'O');
  }
...
// Get indexes
const rowsAndColumns = [], diagonal1 = [], diagonal2 = [], allConditions = [];
  board.forEach( (val, r) => {
    let row = [], column = [];
    board.forEach( (val, c) => {
      row.push([r, c]);
      column.push([c, r]);
      if(r === c) {diagonal1.push([r, c])};
      if(c === (board.length) - r - 1) {diagonal2.push([r, c])};
    });
    rowsAndColumns.push(row);
    rowsAndColumns.push(column);
  });
allConditions.push(...rowsAndColumns, diagonal1, diagonal2);
 
return allConditions.some( combination => helper(combination) );
};
